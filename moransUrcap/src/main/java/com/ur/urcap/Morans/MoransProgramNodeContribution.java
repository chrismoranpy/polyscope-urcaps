...
public class MoransProgramNodeContribution implements ProgramNodeContribution {
	...
	private final MoransProgramNodeView view;
	private final RobotMovement robotMovement;
	private MoranState moranState = new MoranState();

	public MoransProgramNodeContribution(ProgramAPIProvider apiProvider, MoransProgramNodeView view,
										  DataModel model) {
	...
	public void openView() {
		view.updateError(this.moranState.getMessage(), this.moranState.isError());
		...
		if (centerPose != null) {
			robotMovement.requestUserToMoveRobot(centerPose, new RobotMovementCallback() {
				...
				public void onError(MovementErrorEvent event) {
					updateError(new MoranState(getErrorMessage(event.getErrorType())));
				}
			});
		}
	}

	private void updateError(MoranState moranState) {
		this.moranState = moranState;
		view.updateError(moranState.getMessage(), moranState.isError());
	}

	private void clearErrors() {
		this.moranState = new MoranState();
		view.clearErrors();
	}
	...
	private static class MoranState {
